	...
	skyPS(skyPS)
{
	...
	Graphics::Device->CreateRasterizerState(&skyRasterDesc, skyRasterState.GetAddressOf());

	// Create a depth state for the sky
	D3D11_DEPTH_STENCIL_DESC skyDepthDesc = {};
	skyDepthDesc.DepthEnable = true;
	skyDepthDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	skyDepthDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
	Graphics::Device->CreateDepthStencilState(&skyDepthDesc, skyDepthState.GetAddressOf());
...
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	...
	skyVS->SetMatrix4x4("projectionMatrix", camera->GetProjMatrix());
	skyVS->CopyAllBufferData();
	skyPS->SetShaderResourceView("SkyTexture", cubeMapSRV);
