...
float3 Diffuse(float3 normal, float3 lightDir, float3 lightColor, float lightIntensity)
{
    float diffuseAmount = max(dot(normal, lightDir), 0);

    return lightColor * lightIntensity * diffuseAmount;
}

// Function to calculate the specular color
float3 Specular(float3 normal, float3 lightDir, float3 viewDir, float3 lightColor, float roughness)
{
    ...
    {
        spec = pow(saturate(dot(reflection, viewDir)), specExponent);
    }

    return lightColor * spec;
}

// Function to calculate the attenuation factor
...
float3 DirLightPBR(Light light, float3 normal, float3 worldPos, float3 camPos, float roughness, float metalness, float3 surfaceColor, float3 specularColor)
{
    ...
    float3 F;
    float3 spec = MicrofacetBRDF(normal, lightDir, viewDir, roughness, specularColor, F);
