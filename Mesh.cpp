...
/// <param name="vertexNum">num of vertex</param>
/// <param name="indices">index array</param>
/// <param name="indexNum">num of index</param>
Mesh::Mesh(const char* name, Vertex* vertices, unsigned int* indices, size_t vertexNum, size_t indexNum) :name(name)
{
	CreateBuffer(vertices, indices, vertexNum, indexNum);
}
Mesh::Mesh(const char* name, const std::wstring& fileName) :name(name)
{
...
// File input object
	...
	CreateBuffer(&verts[0], &indices[0], vertCounter, indexCounter);
}
...
void Mesh::Draw()
{
	...
	Graphics::Context->DrawIndexed(
		(UINT)indexNum,     // The number of indices to use (we could draw a subset if we wanted)
...
{
	CalculateTangents(vertices, vertexNum, indices, indexNum);
	...
	vbd.ByteWidth = sizeof(Vertex) * (UINT)vertexNum;       // 3 = number of vertices in the buffer
	...
	ibd.ByteWidth = sizeof(unsigned int) * (UINT)indexNum;	// 3 = number of indices in the buffer
