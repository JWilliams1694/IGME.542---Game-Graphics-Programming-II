...
Mesh::Mesh(const char* name, Vertex* vertices, unsigned int* indices, size_t vertexNum, size_t indexNum) :name(name), vbView{}, ibView{}
{
	CreateBuffer(vertices, indices, vertexNum, indexNum);
}
Mesh::Mesh(const char* name, const std::wstring& fileName) :name(name), vbView{}, ibView{}
{
...
void Mesh::CreateBuffer(Vertex* vertices, unsigned int* indices, size_t  vertexNum, size_t  indexNum)
{
	...
	indexBuffer = Graphics::CreateStaticBuffer(sizeof(unsigned int), indexNum, indices);

	//fills out the vertex and index buffer view
	vbView.StrideInBytes = (UINT)sizeof(Vertex);
	vbView.SizeInBytes = (UINT)(sizeof(Vertex) * vertexNum);
